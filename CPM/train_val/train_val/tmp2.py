...
def train_val(model, args):
    ...
    train_loader = torch.utils.data.DataLoader(
        sangi_data.Sangi_Data(train_dir, 8), batch_size=1, shuffle=True,num_workers=config.workers, pin_memory=True)
    ...
    if args.val_dir is not None and config.test_interval != 0:
        ...
        val_loader = torch.utils.data.DataLoader(
            sangi_data.Sangi_Data(val_dir, 8), batch_size=1, shuffle=True, num_workers=config.workers, pin_memory=True)
