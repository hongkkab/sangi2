...
def Config(filename):
    ...
    with open(filename, 'r') as f:
        ...
        print('yaml.load(f)',yaml.load(f))
    
    for x in parser:
        print ('{}: {}'.format(x, parser[x])) #configuration 출력
    #print(parser)
    #{'workers': 4, 'weight_decay': 0.0005, 'momentum': 0.9, 'display': 50, 'max_iter': 62500, 'batch_size': 16, 'test_interval': 50, 'topk': 3, 'base_lr': 4e-06, 'start_iters': 0, 'best_model': 12345678.9, 'lr_policy': 'step', 'policy_parameter': {'gamma': 0.333, 'step_size': 13275}}
    return parser
